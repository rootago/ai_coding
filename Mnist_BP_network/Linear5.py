...
class MyModel:
    ...
    def forward(self,x,label = None):
        for layer in self.layers:
            #x = layer.forward(x)
            x = layer(x) ## linear，sigmoid，softmax有__call__方法
        self.x = x
        if label is not None:
            self.label = label
            loss = -np.mean(label * np.log(x))
            return loss
    ...
    def __call__(self,*args):
        return self.forward(*args)  # args是变参
    '''
    def backward(self):
        G = self.label
...
if __name__ == '__main__':
    ...
    for e in range(epoch):
        for batch_index in range(batch_time):
            ...
            loss = model(x,G)  # MyModel的call方法
            # loss = model.forward(x,G)  
            #print(loss)
