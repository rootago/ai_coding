...
if __name__ == '__main__':
    ...
    #sigmoid2 = Sigmoid()  # 加一层
    #linear3 = Linear(10, 10) # 加一层  循环里的代码不变
    # ...
    softmax1 = Softmax()
    ...
    for e in range(epoch):
        for batch_index in range(batch_time):
            ...
            G = batch_label
            for layer in layers[::-1]: ##倒着取
                G = layer.backward(G)
                
            # G = batch_label
            # G = softmax1.backward(G)
            # G = linear2.backward(G)
            # G = sigmoid1.backward(G)
            # linear1.backward(G)


        x = test_datas
        # x = linear1.forward(x)  # 预测
        # x = sigmoid1.forward(x)
        # x = linear2.forward(x)
        # x = softmax1.forward(x)
        for layer in layers:
            x = layer.forward(x)
        pre = np.argmax(x, axis=1) # 取最大概率的索引
